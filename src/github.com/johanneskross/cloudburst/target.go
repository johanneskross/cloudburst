...
func NewTarget(targetConfiguration TargetConfiguration, factory Factory) *Target {
	...
	return &Target{targetConfiguration.Name, agents, agentChannel, targetConfiguration, factory}
}
...
func startAgents(t *Target, amount int) {
	for i := 0; i < amount; i++ {
		agent := NewAgent(strconv.Itoa(t.Agents.Len()+1)+"("+t.Name+")", make(chan bool), t.Factory.CreateGenerator())
