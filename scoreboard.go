...
type Scoreboard struct {
	TargetId                                                int
	Timing                                                  *Timing
	TotalDropoffs, TotalDropOffWaitTime, MaxDropOffWaitTime int64
	WaitTimeSummaryMap                                      map[string]*WaitTimeSummary
	Scorecard                                               *Scorecard
	OperationResultLock, WaitTimeLock                       sync.Mutex
...
func NewScoreboard(targetId int, timing *Timing) *Scoreboard {
	...
	return &Scoreboard{targetId, timing, 0, 0, 0, waitTimeSummaryMap, scorecard, sync.Mutex{}, sync.Mutex{}, waitTimeChannel, operationResultChannel}
}
...
func (scoreboard *Scoreboard) ModifyOperationResult(operationResult OperationResult) {
	...
	lockStart := time.Now().UnixNano()
	scoreboard.OperationResultLock.Lock()

	dropOffWaitTime := time.Now().UnixNano() - lockStart
	...
	scoreboard.OperationResultLock.Unlock()
...
func (scoreboard *Scoreboard) ModifyWaitTime(waitTime WaitTime) {
	...
	scoreboard.WaitTimeLock.Unlock()
